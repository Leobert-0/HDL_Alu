...
module alu_module # (parameter N = 8)(
    result, carry, zero_flag, overflow_flag, neg_flag, x, y, opcode
    );
    input [N-1:0] x, y;
    input [3:0] opcode;
    ...
    wire [N-1:0] add_sum;
    add_module #(N) addition (add_carry, add_sum, add_overflow, x, y);
    
    wire sub_borrow, sub_overflow;
    wire [N-1:0] sub_diff;
    subtract_module #(N) subtraction (sub_borrow, sub_diff, sub_overflow, x, y);
    ...
    shl_module #(N) shift_left (shl_carry, shl_result, x);
    
    wire shr_carry;
    wire [N-1:0] shr_result;
    shr_module #(N) shift_right (shr_carry, shr_result, x);
    
    wire inc_carry, inc_overflow;
    wire [N-1:0] inc_result;
    inc_module #(N) increment (inc_carry, inc_result, inc_overflow, x);
    
    wire dec_borrow, dec_overflow;
    wire [N-1:0] dec_result;
    dec_module #(N) decrement (dec_borrow, dec_result, dec_overflow, x);
    
    always @ (opcode, x, y) begin
        case(opcode)
            4'b0000: begin
                result = add_sum;
                carry = add_carry;
